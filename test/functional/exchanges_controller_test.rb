...
class ExchangesControllerTest < ActionController::TestCase
  ...
  test "employee can access the exchanges report" do
    post :report, :graph_start => '21/01/2011', :graph_ending => '21/12/2012', :graph => {:promotion_id => 1 }
    assert_response 200
  ...
  test "employee can access the exchanges report even if dates are not valids" do
    post :report, :graph_start => 'blabla', :graph_ending => 'blabla', :graph => {:promotion_id => 1 }
    assert_response 200
